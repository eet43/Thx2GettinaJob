...
public interface ExamRepository extends JpaRepository<Exam, Long>, ExamRepositoryCustom {
        ...
        @Query("SELECT new KHOneTop.Thx2GettinaJob.exam.dto.NearExamInfo(e.id, e.name, e.issuer, e.url, MIN(ets.regEndDate), MIN(ets.addRegEndDate)) FROM Exam e JOIN e.examTimeStamp ets WHERE (ets.regEndDate > CURRENT_TIMESTAMP OR ets.addRegEndDate > CURRENT_TIMESTAMP) AND e.id IN :examIds GROUP BY e.id ORDER BY MIN(LEAST(ets.regEndDate, ets.addRegEndDate)) ASC")
